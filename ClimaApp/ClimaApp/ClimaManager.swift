...
struct ClimaManager {
    ...
    func parseJSON(climaData: Data) -> ClimaModelo?{
        ...
        do{
            ...
            var descripcion = dataDecodificada.weather[0].description
            let first = String(descripcion.prefix(1)).capitalized
            let other = String(descripcion.dropFirst())
            descripcion = first+other
